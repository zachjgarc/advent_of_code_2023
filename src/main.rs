...
use std::fs;
use std::{io::{self, Read, Write}, time::{Instant, Duration}};
use std::path::Path;
...
fn main() {
    ...
    if let Some(add_matches) = matches.subcommand_matches("add") {
        let day_str: &String = add_matches
            .get_one::<String>("day")
        ...
        let use_test: bool = *matches.get_one::<bool>("test").unwrap_or(&false);
...
fn add_new_day(day: u32) {
    ...
    let day_dir: String = format!("src/days/day_{}", day_str);
    ...
    let mod_path: String = format!("{}/mod.rs", day_dir);
    ...
    // update mod.rs
    let mod_path: &str = "src/days/mod.rs";
    let mod_entry: String = format!("pub mod day_{};\n", day_str);
    fs::OpenOptions::new()
        .append(true)
        .open(mod_path)
    ...
    if !Path::new(&inputs_dir).exists() {
        if let Err(e) = fs::create_dir(&inputs_dir) {
            eprintln!("Failed to create directory  {}: {}", inputs_dir, e);
            std::process::exit(1);
        }
    }

    let test_inputs_dir = "src/test inputs";
    if !Path::new(&test_inputs_dir).exists() {
        if let Err(e) = fs::create_dir(&test_inputs_dir) {
            eprintln!("Failed to create directory {}: {}", test_inputs_dir, e);
            std::process::exit(1);
        }
    }
